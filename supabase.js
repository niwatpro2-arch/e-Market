...
// Browser code intentionally uses only the public publishable key.
...
async function loadStoreProducts() {
  ...
  const { data, error } = await sb.from('products').select('id,name,description,price,image_url,stock,is_active,category_id,categories(name)').eq('is_active', true).order('created_at', { ascending: false })
...
async function createStoreOrder({ customerName, customerPhone, fulfillmentMethod, items }) {
  ...
  if (error) throw new Error(error.message || 'ไม่สามารถสร้างคำสั่งซื้อได้')
...
async function findOrder(orderNumber, customerPhone) {
  if (!orderNumber || !customerPhone) return null
  const response = await fetch(`${SUPABASE_URL}/functions/v1/track-order`, {
    ...
    body: JSON.stringify({ orderNumber: orderNumber.trim(), customerPhone: customerPhone.trim() })
  })
  const body = await response.json().catch(() => ({}))
  if (response.status === 404) return null
  if (!response.ok) throw new Error(body.error || 'ไม่สามารถค้นหาออเดอร์ได้')
  return body.order ? { ...body.order, items: body.items || [] } : null
}

window.NongMaiNgamSupabase = { getSupabase, loadStoreProducts, loadStoreCategories, createStoreOrder, findOrder, makeOrderNumber, url: SUPABASE_URL }
